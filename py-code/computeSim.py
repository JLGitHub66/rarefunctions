...
alloc_wv = normalize(model.wv["_alloc"])
...
for line in ffuntionSubword.readlines():
    if line != "\n":
        if i % 3 == 1:
            ...
            keyword = line.strip("\n")
            keyword_wv = normalize(model.wv[keyword])
            sim = np.dot(alloc_wv, keyword_wv)/(np.linalg.norm(alloc_wv) * np.linalg.norm(keyword_wv))
            if keyword not in api_sims.keys():
                api_sims[keyword] = sim
    i += 1
print('output keyword and similirity:')
...
# print("start DBScan.")
# db = DBSCAN(eps=0.8, min_samples=100).fit(keyvecList)
# labels = db.labels_
