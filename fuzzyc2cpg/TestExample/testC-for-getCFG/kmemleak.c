				 ...
				 size_t len, bool ascii)
{
	if (seq)
		...
		print_hex_dump(KERN_WARNING, KBUILD_MODNAME ": " "    ", prefix_type,
			    ...
			    struct kmemleak_object *object)
{
	...
	len = min_t(size_t, object->size, 2 * 16);

	warn_or_seq_printf(seq, "  hex dump (first %zu bytes):\n", len);
	kasan_disable_current();
	warn_or_seq_hex_dump(seq, DUMP_PREFIX_NONE, 16,
			     1, kasan_reset_tag((void *)ptr), len, 1);
...
{
	return (color_white(object) && object->flags & (1 << 0)) &&
...
static struct kmemleak_object *lookup_object(unsigned long ptr, int alias)
{
	...
	while (rb) {
		...
		else if (object->pointer == ptr || alias)return object;
		else {
...
{
	return stack_trace_save(trace, 16	, 2);
}
...
{
	unsigned long *ptr;
	unsigned long *start = PTR_ALIGN(_start, sizeof(void *));
	unsigned long *end = _end - (sizeof(void *) - 1);
	unsigned long flags;
	unsigned long untagged_ptr;
...
static void kmemleak_scan(void)
{
	...
	list_for_each_entry_rcu(object, &object_list, object_list) {
		raw_spin_lock_irqsave(&object->lock, flags);
		if (color_white(object) && (object->flags & (1 << 0))
...
static int kmemleak_scan_thread(void *arg)
{
	 ...
	 */
	if (first_run) {
		signed long timeout = msecs_to_jiffies(60	 * 1000);
		first_run = 0;
...
static int kmemleak_seq_show(struct seq_file *seq, void *v)
{
	...
	raw_spin_lock_irqsave(&object->lock, flags);
	if ((object->flags & (1 << 1)) && unreferenced_object(object))
...
void __init kmemleak_init(void)
{
	...
	jiffies_min_age = msecs_to_jiffies(5000	);
	jiffies_scan_wait = msecs_to_jiffies(600	 * 1000);
