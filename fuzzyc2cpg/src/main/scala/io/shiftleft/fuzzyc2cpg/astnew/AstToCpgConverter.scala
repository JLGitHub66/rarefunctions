...
// HJJ: Add a helper for global vars.
class AstToCpgConverterHelper {
  private var map = Map.empty[String, String]
  def record(v: String, t: String): Unit = {
    map += (v -> t)
  }
  def lookup(v: String): Option[String] = {
    map.get(v)
  }
}
...
class AstToCpgConverter[NodeBuilderType, NodeType, EdgeBuilderType, EdgeType](
    ...
    helper: AstToCpgConverterHelper)
  ...
  override def visit(argument: Argument): Unit = {println("\nArgument: " + argument.getExpression.getClass)
    ...
    Option(astUnary.getChild(0)) match {
      case Some(_) =>
        val operatorMethod = astUnary.getChild(0).getEscapedCodeStr match {
          ...
          case _    => astUnary.getChild(0).getEscapedCodeStr
        }
  ...
  override def visit(astIdentifier: Identifier): Unit = {
    ...
    val identifierTypeName = variableOption match {
      case Some((_, variableTypeName)) =>
        variableTypeName
      case None => {
        //HJJ: the following line are added for type retrieval of global
        helper.lookup(identifierName) match {
          case Some(tn) =>
            tn
          case None => {
            Defines.anyTypeName
      ...
      // HJJ: Replace the following with the above lines to make global vars typed.
      // case None =>
      //  Defines.anyTypeName
    }
...
println("\nidentifierDecl " + declTypeName)
    if (identifierDecl.isTypedef) {
      ...
      println("\n parent is class def => " + cpgMember.toString)
    } else {
      ...
      if (!scope.isEmpty) {
        ...
        // HJJ: We want to remember the type of global variables.
        val globalName = identifierDecl.getName.getEscapedCodeStr
        helper.record(globalName, declTypeName)
      }
    }
  }
  ...
  override def visit(astInstance: InstanceExpression): Unit = {
    val cpgInstance = createCallNode(astInstance, "instanceofexpression")
    addAstChild(cpgInstance)
  }
  ...
  private def acceptChildren(node: AstNode, withArgEdges: Boolean = false): Unit = {
    node.getChildIterator.forEachRemaining { child =>
      ...
      println("\n Child = " + child.getEscapedCodeStr + " -> " + child.getTypeAsString)
      child.accept(this)
    }
  }
