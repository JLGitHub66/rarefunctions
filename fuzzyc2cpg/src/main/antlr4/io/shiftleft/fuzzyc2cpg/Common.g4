...
//_input.LA(1);该方法可以在不移动文件指针的情况下获取输入流的下一个字符（char型，或者理解为int型的ascii码也可）。https://blog.csdn.net/qq_35318180/article/details/82056763
// consume 函数的作用是，让字符串指针向前挪一个字符。https://juejin.cn/post/6844903701463695374
@parser::members {
...
access_specifier: ('public' | 'private' | 'protected' | 'class');
...
unary_expression: inc_dec cast_expression
                ...
                | sizeof_expression
                | ((identifier '->')*)? postfix_expression  //在这里添加了((identifier '->')*)?    因为面对p->function()这样的，函数名的导出有错误。2022年7月29日
                ;

new_expression: '::'? NEW type_name '[' conditional_expression? ']' 
              | '::'? NEW type_name '(' expr? ')'
              | '::'? NEW postfix_expression '(' expr? ')'
              ;
...
postfix_expression: postfix_expression '[' expr ']' #arrayIndexing
                  ...
                  | postfix_expression 'instanceof' expr ('<' function_argument_list? '>')?  #instance
                  | primary_expression # primaryOnly
                  ;

function_argument_list: ( function_argument (',' function_argument)* )?;
function_argument: assign_expr (COMMENT|LINE_COMMENT)?;


primary_expression: identifier (COMMENT|LINE_COMMENT)?| constant (COMMENT|LINE_COMMENT)?| '(' expr ')';
