...
/* miner: indicates whether there are command line options started with "-aminer-" */
static int has_miner_options = 0;
static int miner_argc = 0;
static char **miner_argv = NULL;
...
store_arg (const char *arg, int delete_always, int delete_failure)
{
  ...
  if (delete_always || delete_failure)
    ...
    if(has_miner_options)
  {
    int len = strlen(arg);
    if(len >= 3 && arg[len-1] == '1' && arg[len-2] == 'c' && arg[len-3]=='c')
    {
      const char *miner_prefix = "-aminer-";
      int i;
      for(i = 0; i < miner_argc; i ++)
        if(strncmp(miner_argv[i], miner_prefix, strlen(miner_prefix)) == 0)
          argbuf.safe_push(miner_argv[i]);
      has_miner_options = 0;
    }
  }
}
...
#if 0
static void may_reserve_backups(int argc, char **argv)
{
  system("which gcc > /tmp/temp-miner-store-compiling-commands.txt");
  char buf[1024];
  FILE *tmp_fp = fopen("/tmp/temp-miner-store-compiling-commands.txt", "r");
  fgets(buf, 1024, tmp_fp);
  buf[strlen(buf) - 1] = '\0';
  fclose(tmp_fp);
  system("rm /tmp/temp-miner-store-compiling-commands.txt");

  const char *command_name = "../../config/save-command.txt";
  char *gcc_path = buf;
  int len = strlen(gcc_path), i;
  for(i = len - 1; i >= 0; i --)
  {
    if(gcc_path[i] == '/')
      break;
  ...
  if (i < 0)
    return;
  char *file_path = (char *)xmalloc(strlen(command_name) + (i + 1) + 1);
  snprintf(file_path, (i + 2), "%s", gcc_path);
  sprintf(file_path + (i + 1), "%s", command_name);
  ...
  buf[strlen(buf) - 1] = '\0';
  FILE *target_fp = fopen(buf, "a");
  if(target_fp == NULL)
    return;
  for(i = 1; i < argc; i ++)
    fprintf(target_fp, "%s ", argv[i]);
  fprintf(target_fp, "\n");
}
#endif
/* extract command options specific for my miner, if there are none, keep them as they are */
static void set_miner_options(int *in_argc, char ***in_argv)
{
  char *libvar = getenv("SELF_COMPILE");
  if(libvar != NULL && strcmp(libvar, "1") == 0)
    return;
  int i, argc = *in_argc;
  const char *miner_prefix="-aminer-";
  char **argv = *in_argv;
  ...
  for (i = 0; i < argc; i ++)
  {
    if(strcmp(argv[i], "-.i") == 0)
      continue;
    if(strncmp(argv[i], miner_prefix, strlen(miner_prefix)) == 0)
    {
      if(has_miner_options == 0)
      {
        has_miner_options = 1;
        miner_argc = argc;
        miner_argv = argv;
  ...
  {
    int argcp = 0;
    char **argvp = (char **)xmalloc((argc - num) * sizeof(char *));
    for(i = 0; i < argc; i ++)
    {
      if(strcmp(argv[i], "-.i") == 0)
        continue;
  ...
  {
    /* If no miner options are set, we suppose the user wan't to extract .i files */
    int argcp = 0;
    char **argvp = (char **)xmalloc((argc + 1) * sizeof(char *));
    for(i = 0; i < argc; i ++) {
      if(strcmp(argv[i], "-.i") == 0)
        continue;
      argvp[argcp ++] = argv[i];
    }
    argvp[argcp ++] = (char *)"-save-temps=obj";
    *in_argc = argcp;
    *in_argv = argvp;
  }
#endif
...
{
  /* miner: check whether there are options for our miner specially */
  set_miner_options(&argc, &argv);
  old_argv = argv;
